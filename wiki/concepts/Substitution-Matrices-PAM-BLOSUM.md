...
## 4. Static Matrices vs. Position-Specific Scoring Matrices (PSSMs)

While PAM and BLOSUM are **position-invariant** (the score for $L \leftrightarrow V$ is identical at every sequence position), [[Position-Specific-Iterated-BLAST|PSSMs]] generated by [[PSI-BLAST]] compute position-dependent log-odds scores:
- Strictly conserved positions in active catalytic sites or structural folds receive sharp penalties for divergence.
- Highly variable loop regions receive lenient substitution scores.

---

## 5. Relationship to Karlin-Altschul Statistics
